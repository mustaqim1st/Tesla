# Made with ❤ by @lordbeastop
# Join tele channel for update t.me/lordbeastop
import requests
import time
...
def print_welcome_message():
    ...
    print(Fore.YELLOW + Style.BRIGHT + "Free Consultancy Join Telegram Channel: https://t.me/lordbeastop")
    print(Fore.BLUE + Style.BRIGHT + " Ton Address -  UQCTPmyv7A3NOpJtWTUHK8PrMrWE1eWU9ZdSuBbE4dHcBlzz")
...
def get_headers(token):
    return {
        ...
        'pragma': 'no-cache',
        'priority': 'u=1, i',
        'referer': 'https://www.yescoin.gold/nPX9hQ',
...
def process_account(token):
    ...
    if squad_info and squad_info['data']['isJoinSquad']:
        ...
        print(f"{Fore.YELLOW + Style.BRIGHT}\r[ Squad ] :  Join Squad. Joining LordBeast.", end="", flush=True)
        time.sleep(3)
        join_result = join_squad(token, "t.me/lordbeastop")
        if join_result:
            print(f"{random.choice(available_colors) + Style.BRIGHT}\r[ Squad ] : Welcome  {nickname} - LordBeast !      ", flush=True)
        else:
            print(f"{random.choice(available_colors) + Style.BRIGHT}\r[ Squad ] : Failed to join LordBeast.", flush=True)
        ...
        if recover_coin_pool(token):
            ...
            if collect_info:
                ...
                if coin_pool_left_count > 0:
                    ...
                    if collect_result and collect_result.get('code') == 0:
                        collected_amount = collect_result['data']['collectAmount']
                        print(f"{Fore.GREEN + Style.BRIGHT}\r[ Collect ] : Successfully collected {collected_amount} coin", flush=True)
                    else:
...
def main():
    ...
    while True:
        ...
        print(f"\n{random.choice(available_colors)+Style.BRIGHT}========={Fore.WHITE+Style.BRIGHT}All accounts successfully processed{Fore.GREEN+Style.BRIGHT}=========", end="", flush=True)
        import sys
        waktu_tunggu = 15  # 5 menit dalam detik
        for detik in range(waktu_tunggu, 0, -1):
            sys.stdout.write(f"\r{Fore.CYAN}Waiting for the next claim time in {Fore.CYAN}{Fore.WHITE}{detik // 60} minute {Fore.WHITE}{detik % 60} seconds")
            sys.stdout.flush()
            time.sleep(1)
...
def parse_arguments():
    ...
    if args.fill is None:
        fill_input = input("Do you want to upgrade fill rate? (y/n, default n): ").strip().lower()
